...
export default function AnalyticsPage({ dersler, harfNotlari, stats }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {donemStats.map((d) => (
              <div key={d.donem} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: `1px solid ${tokens.border}` }}>
                <span style={{ fontSize: 13, color: tokens.textSecondary }}>Dönem {d.donem}</span>
                <span style={{ fontSize: 13, color: tokens.muted }}>{d.gecen}/{d.dersSayisi} geçti</span>
        ...
        <Card title="Harf Notu Dağılımı">
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {gradeDistribution.length === 0 && <div style={{ fontSize: 13, color: tokens.muted }}>Henüz veri yok.</div>}
      ...
      <Card title="Akademik Performans Metrikleri">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16 }}>
          <Metric label="Genel GPA" value={stats.gano} tokens={tokens} />
          <Metric label="Alınan Kredi" value={stats.alinanKredi} tokens={tokens} />
          <Metric label="Geçilen Kredi" value={stats.gecenKredi} tokens={tokens} />
          <Metric label="Tamamlanma" value={`${stats.tamamlanmaOrani}%`} tokens={tokens} />
        </div>
      </Card>
    </div>
  );
